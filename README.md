...
-   Proper interaction with KOReader in general requires a recent version of KOReader (i.e., >= 2015.11-1735).
    -   As far as for successfully restarting nickel on exit is concerned, I'd also recommend running a current FW version (last tested on FW 4.7.x - 4.17.x).  
...
-   Since FW 4.17, Nickel *will* index content found in *nix hidden directories by default. Thankfully, this behavior is [configurable](https://www.mobileread.com/forums/showpost.php?p=3892463&postcount=10).  
    Note that the "One Click Packages" [installers](https://github.com/NiLuJe/kfmon/blob/1b77bfff61fc26882a0d2b1b1de7025060949f9d/tools/install.sh#L85) take care of handling that setting automatically ;).
